...
type Props = {
  searchParams: Promise<{ player?: string; q?: string; sort?: string }>;
};
...
export default async function GlobalCharactersPage({ searchParams }: Props) {
  ...
  const [catalog, batting, managers] = await Promise.all([
    getGlobalCharacterCatalog(),
  ...
  const sorted = sortCharacterLibrary(sortable, batting, sort);
